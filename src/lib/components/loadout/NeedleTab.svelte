...
<div class="w-full">
    <div class="page md:px-8 px-10">
        ...
        {#each needle_info as type} 
            <div class="px-8 mb-4" onclick={ () => { user_info.current_needle = type; } }>
                <NeedleSlot needle_type={type} is_selected={user_info.current_needle === type}/>
            </div>
        {/each}
        </div>
    </div>
